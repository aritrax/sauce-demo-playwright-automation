...
test("TC06 Verify user lands on Inventory page after login @smoke @regression" , async({poManager})=>
{
    const inventoryPage = poManager.getInventoryPage() ;
    await inventoryPage.verifyURL();
})

test("TC07 Verify all products are displayed @smoke @regression", async ({ poManager }) => {
    const inventoryPage = poManager.getInventoryPage();
    await inventoryPage.verifyallInventory()
    
});

test("TC08 Verify product information — name, price, image @regression" , async({poManager})=>
{
    const inventoryPage = poManager.getInventoryPage();
    await inventoryPage.verifyProductDeatils()
})

test("TC09 Verify product details page opens correctly @regression" , async({poManager})=>
{
    const productDeatilsPage = poManager.getProductDetailsPage();
    await productDeatilsPage.viewProductDetails()

})
test("TC10 Verify product sorting — Name A → Z @regression" , async({poManager , page})=>
{
    const inventoryPage = poManager.getInventoryPage();
...
test("TC11 Verify product sorting — Name Z → A @regression" , async({poManager , page})=>
{  
    const inventoryPage = poManager.getInventoryPage();
...
test("TC12  Verify product sorting — Price Low → High @regression" , async({poManager , page})=>
{
    const inventoryPage = poManager.getInventoryPage();
